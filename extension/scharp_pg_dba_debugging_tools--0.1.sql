...
\echo Use "CREATE EXTENSION [ IF NOT EXISTS ] scharp_pg_dba_debugging_tools WITH SCHEMA tools [ CASCADE ]" to load this file. \quit

CREATE SCHEMA IF NOT EXISTS @extschema@;

GRANT USAGE ON SCHEMA @extschema@ TO PUBLIC;

CREATE OR REPLACE FUNCTION @extschema@.page_header (table_name regclass)
RETURNS TABLE (
...
GRANT EXECUTE ON FUNCTION @extschema@.page_header (table_name regclass) TO PUBLIC;

CREATE OR REPLACE FUNCTION @extschema@.heap_page_item_attrs (
...
GRANT EXECUTE ON FUNCTION @extschema@.heap_page_item_attrs_details (table_name regclass) TO PUBLIC;

CREATE OR REPLACE FUNCTION @extschema@.page_count (table_name regclass)
RETURNS int4
AS
$body$
...
GRANT EXECUTE ON FUNCTION @extschema@.page_count (table_name regclass) TO PUBLIC;
